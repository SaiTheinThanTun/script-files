...
#transformation from 5 to 15 year age group
tmp <- Men.period.decom
tmp1 <- colSums(tmp[row.names(tmp) %in% names(table(dat$agegrp))[1:3],])
tmp2 <- colSums(tmp[row.names(tmp) %in% names(table(dat$agegrp))[4:6],])
tmp3 <- colSums(tmp[row.names(tmp) %in% names(table(dat$agegrp))[7:9],])
tmp <- rbind(tmp1,tmp2, tmp3, tmp[nrow(tmp),])
Men.period.decom <- tmp


Women.period.decom <- decomList.Women.all.2007.2011[,c(8:18)]*12
#transformation from 5 to 15 year age group
tmp <- Women.period.decom
tmp1 <- colSums(tmp[row.names(tmp) %in% names(table(dat$agegrp))[1:3],])
tmp2 <- colSums(tmp[row.names(tmp) %in% names(table(dat$agegrp))[4:6],])
tmp3 <- colSums(tmp[row.names(tmp) %in% names(table(dat$agegrp))[7:9],])
tmp <- rbind(tmp1,tmp2, tmp3, tmp[nrow(tmp),])
Women.period.decom <- tmp


period.decom <- rbind(Women.period.decom,Men.period.decom)
period.decom <- period.decom[,which(colSums(period.decom)!=0)]
age <- rep(names(table(dat$agegrp15)),2)
sex <- rep(c("All Women","All Men"), each=length(table(dat$agegrp15)))
...
labdat_hiv <- data.frame(x=4, y=4, lab=c(paste("Total difference, injury:",round(12*sum(decomList.Men.Positive.Negative[,8:18]),2),"months"), paste("Total difference, injury:",round(12*sum(decomList.Women.Positive.Negative[,8:18]),2),"months")), sex=c("Men","Women"))
labdat_hiv2 <- data.frame(x=4, y=4, lab=c(paste("Total difference, all-cause:",round(sum(decomList.Men.Positive.Negative[,7]),2),"years"), paste("Total difference, all-cause:",round(sum(decomList.Women.Positive.Negative[,7]),2),"years")), sex=c("Men","Women"))
